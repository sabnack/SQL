...
--Виділяються два різновиди мови SQL: PL-SQL та T-SQL. PL-SQL використовується у таких СУБД як Oracle та MySQL. 
...
--Ще два типи TEXT та NTEXT є застарілими і тому їх не рекомендується використовувати. 
...
--Атрибут IDENTITY дозволяє зробити стовпець ідентифікатором. 
...
--Щоб вказати, чи може стовпець приймати значення NULL, при визначенні стовпця можна задати атрибут NULL або NOT NULL . 
...
--Атрибут DEFAULT визначає значення за промовчанням для стовпця. 
...
--Ключове слово CHECK визначає обмеження для діапазону значень, які можуть зберігатися в стовпці. 
